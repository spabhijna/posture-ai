    ...
    def check_pose(self, keypoints):
        ...
        for rule in self.rules:
            if rule['type'] == 'angle':
                ...
                if c == 'Vertical':
                    c = [keypoints[b][0],keypoints[b][1]-100] # Virtual vertical point
                else:
                    c = keypoints[c]
                ...
                dist1 = np.linalg.norm(np.array(keypoints[pair1[0]]) - np.array(keypoints[pair1[1]]))
                dist2 = np.linalg.norm(np.array(keypoints[pair2[0]]) - np.array(keypoints[pair2[1]]))
                ...
                angle = self.calculate_vector_angle(
                    keypoints[vec1[0]], keypoints[vec1[1]],
                    keypoints[vec2[0]], keypoints[vec2[1]]
                )
                if not self._check_threshold(angle, rule['threshold']):
                    issues.append(f"{rule['name']} incorrect ({angle:.1f} degrees)")
