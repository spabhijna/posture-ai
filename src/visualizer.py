        ...
        for i, kp in enumerate(vertices.xy):
            if len(kp) > 0 and i < len(labels):  # Ensure keypoints and labels exist
                ...
                total_height = (len(lines) * text_height + 
                               (len(lines) - 1) * spacing if len(lines) > 1 else text_height)
                ...
                max_line_width = max([cv2.getTextSize(line, font, font_scale, thickness)[0][0] 
                                     for line in lines])
                x = int(head_pos[0])
                ...
                bottom = y_start + (len(lines) - 1) * (text_height + spacing) + text_height + padding
                cv2.rectangle(annotated_frame, (left, top), (right, bottom), (0, 0, 0), -1)  # Black background

                # Draw each line of text
                y = y_start
                for line in lines:
                    cv2.putText(annotated_frame, line, (x, y), font, font_scale, (0, 255, 0), thickness)
